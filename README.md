...
## Report (LaTeX)
The report source is `acm_report.tex`. Compile with a LaTeX install (e.g., TeX Live):
`pdflatex acm_report.tex` (run twice), or `latexmk -pdf acm_report.tex` if available.
